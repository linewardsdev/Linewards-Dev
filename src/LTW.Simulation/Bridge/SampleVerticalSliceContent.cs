...
public static class SampleVerticalSliceContent
{
    ...
    private static readonly ContentId[] DefensiveBuildOrder =
    {
        ...
        ThornSnareTowerId,
        BarricadeTowerId,
        // Appended rather than inserted: the note above warns that early slots set bot
        // opening timing, so a new tower goes at the end where it can only affect a bot
        // that has already built nine.
        TwinCrescentTowerId
    };
    ...
    public static ContentCatalog Create()
    {
        return new ContentCatalog(
            ...
            new[]
            {
                ...
                new TowerDefinition(TwinCrescentTowerId, "Twin Crescent Ward", new Gold(30), rangeCells: 2, damage: 6, attackCooldownTicks: 6, categoryIndex: 0),
