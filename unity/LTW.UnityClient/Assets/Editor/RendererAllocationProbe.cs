...
{
    /// <summary>
    /// Measures how many managed bytes <see cref="UnityVerticalSliceRenderer"/> allocates per frame,
    /// on a deterministic mid-match board, with the renderer enabled versus disabled in the same play
    /// session so the difference is attributable to the renderer alone.
    /// </summary>
    /// <remarks>
    /// Written for OPEN_ITEMS item 24 and kept because that item's ledger row cites its numbers: a
    /// figure nobody can re-run is a claim, not a measurement. It is not a pass/fail gate and nothing
    /// runs it automatically — it prints and exits 0 either way, like the motion capture next to it.
    ///
    /// Three phases, all on the same board:
    ///
    ...
    /// - RUNNING, renderer on, over a fixed number of ticks at the shipped 4/s, for the figure a real
    ///   match sees. Its frame count is reported, since the frames-per-tick ratio moves with how fast
    ///   each frame is.
    ///
    /// Run with:
    ...
    public static class RendererAllocationProbe
    {
            ...
            switch (phase)
            {
                ...
                case Phase.PausedOff:
                    if (StepFrames(driver, renderer, out var pausedOffBytes))
                    {
                        ...
                        // back on, the match ticking at the shipped 4/s.
                        driver.enabled = true;
                        driver.TogglePause();
        ...
        private static void Finish(string error)
        {
            ...
            Debug.Log(string.Format(
                ...
                "renderer-off {4:F0} B/frame, renderer share {5:F0} B/frame ({6:F1} KB/frame) | RUNNING renderer-on {7:F0} B/frame",
                measuredCreeps, measuredTowers, measuredActiveObjects, onA, off, onA - off, (onA - off) / 1024d, onB));
