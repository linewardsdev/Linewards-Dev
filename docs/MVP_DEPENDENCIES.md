...
| --- | --- | --- | --- | --- | --- |
| FREE - Stylized Weapons | Blink, imported from Unity Package Manager / Asset Store local cache | `unity/LTW.UnityClient/Assets/ThirdParty/StylizedWeaponKit/Art/Weapons/Stylized/` | Rapid production-art reference and candidate tower/builder silhouette parts: swords, staves, shields, polearms, scythes, hammers, axes, daggers, musket | Imported on 2026-07-15 from local `.unitypackage`; demo scene content removed after import cleanup. Treat as third-party Unity Asset Store content and verify license/distribution assumptions before store release. | Low to medium while used only as visual candidates; higher if prefabs become production tower assets. |
...
| PlayFab Multiplayer Servers (MPS) | Microsoft (Azure) | PlayFab Terms of Service; a free evaluation tier (750 Dasv4 core-hours/month, 24 simultaneous cores, confirmed against Microsoft Learn docs as of 2026-09-05) covers this project's current scale, then usage-based consumption pricing per VM-core-hour plus egress | `com.playfab.csharpgsdk` 0.11.210519 (last published 2021 — confirmed loading/running cleanly on net10.0, see MP-07) added to `src/LTW.MatchServer/LTW.MatchServer.csproj`, alongside a direct `Newtonsoft.Json` 13.0.3 pin (the GSDK package's own transitive 11.0.2 has a known high-severity vulnerability, NU1903) | Hosts `LTW.MatchServer` — MULTIPLAYER_SEATS_AND_AUTHORITY.md's MP-07, chosen over a generic Azure Container Apps deployment since PlayFab is already this project's identity provider | Medium: a real, if old, runtime NuGet package this process's process-exit-on-match-end lifecycle now depends on (see MP-07's "Full GSDK integration"). Removing it means reverting `Program.cs`'s `mps` mode entirely and re-deciding hosting from scratch; the `standalone` mode (today's default) has no dependency on it at all. |
...
| ~~Container image and registry~~ | Built and live-verified 2026-09-05 — `src/LTW.MatchServer/Dockerfile`, a real create-match/WebSocket-join round trip proven across a real Docker NAT boundary. See MP-07. | — |
| Cloud host or managed game hosting | Decided 2026-09-05 — PlayFab Multiplayer Servers, not a self-managed Azure container host. GSDK integration landed; live allocation not yet verified. See "Third-Party Runtime Services" above and MP-07. | Live GSDK lifecycle verification against `LocalMultiplayerAgent` or a real title build. |
