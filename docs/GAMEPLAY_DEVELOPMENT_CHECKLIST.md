...
The first art-upgrade pass also aligns board material bands, endpoint halos, gate trim, and the default orthographic camera, optional active-lane camera framing, outside-lane gutters, corner anchors, gutter ownership badges, and directional gutter tick marks to the side-by-side north-south lane layout so the LTW-style vertical flow stays readable.
