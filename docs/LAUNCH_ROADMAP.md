...
| 9 | **Graphics Wave 1 — AO done, LODs wired, normal maps at a fork** — *worked 2026-08-04/06* | **AO bound on all 30 roles** (was 2 of 119); evidence in `screenshot-reviews/stylized-shader-20260801/after_ao_bound_roster.png`. **LODs wired on all 30 prefabs** — every role decimated to 50%/25% (`tools/art/make_all_lods.py`) and given a LODGroup (`AuthorLodGroups`), closing open item 15's last sub-item; before this the two proof meshes were referenced by nothing. **Normal maps** now have a route — bake LOD0 onto LOD2 — but the levels share LOD0's material, and a high-to-low map is only correct on the low mesh, so this needs per-LOD materials first. That is a decision, not a task | ~~2–4 d~~ AO + LODs done · normals need a decision |
