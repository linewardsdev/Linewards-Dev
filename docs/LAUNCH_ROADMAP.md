...
| 9 | **Graphics Wave 1 — AO landed roster-wide; normal maps deferred by decision** — *re-checked 2026-08-06* | All **30 roles** now bake and bind an occlusion map (was 2 of 119 on 4 Aug), and all **30 prefabs** carry LOD groups. The stylized shader has a normal-map slot wired and unused. **Normal maps are deliberately not being done now:** a high-to-low bake is only correct on the low mesh, and all three LOD levels currently share one material per role, so shipping one would need ~60 new materials and more SRP-batcher breaks for detail that is close to invisible at the 46–105px units occupy. Owner agreed 2026-08-06 to bank the LOD win and revisit only if LOD popping is visible in play | ~~2–4 d~~ deferred |
