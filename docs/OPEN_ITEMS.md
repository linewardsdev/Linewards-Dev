...
environment can act on — see the "Resolved 2026-09-02" ledger and
`screenshot-reviews/render-review-20260901/README.md`. Four small items remain, all judged
from the Wave 5 frames:

- **Label-on-label stacking on a busy leak row** — `37-leak-row-labels-seq-01`: "+25" and
  "−5 LIVES" overlap slightly when they land in the same 0.2 s merge window at the gate. Not
  occlusion (that is fixed and proven); the stack offset needs one more slot. ¼ day.
- **Hit flash fires before the dart lands** — the core renderer's immediate
  `SpawnEffect(hitPosition)` in `RenderEvents` still fires at event time, 0.12 s before the
  Wave 5 projectile arrives and spawns its own impact burst. Move it to the projectile's
  arrival (a hook in `UnityVerticalSliceRenderer.cs`, which the VFX agent did not own). ¼ day.
- **Elimination and victory lane cues are still crossed beams** — lane-level, not combat;
  cosmetic. Replace with a ring pulse like `SpawnCellFrameCue` now does. ¼ day.
- **Control ward base dish** — the translucent violet pancake under the Control ward is the
