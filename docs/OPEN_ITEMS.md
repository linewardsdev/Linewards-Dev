...
| Item | Commit | Outcome |
| --- | --- | --- |
| Render review Wave 1 | (this commit) | Six blockers from the 2026-09-01 live-capture graphics audit (`screenshot-reviews/render-review-20260901/README.md`), worked in parallel and verified with one compile, 335/335 tests, and a re-run of `GraphicsAuditCaptureRunner` at the same frames. **#1 Twin Crescent white:** not an unbound slot — the body material referenced a texture Unity had extracted into a `.fbm/` folder that `.gitignore` drops, so the binding dangled in every checkout but the one it was authored on. The bake (byte-identical to Arrow's, the kitbash keeps Arrow's UVs) is now committed under `TwinCrescent/AIDrop/*_Textures/` and bound; renders textured. **#3 Placement ghost:** the real prefab was resolving all along — it was drawn with `Sprites/Default`, unlit and ZWrite off, so a Control ward's base and ring stacked into two 93%-opaque violet discs; now a lit URP transparent material with ZWrite forced on, root at the placed-tower base height, `OwnerTrim`/`RangeHalo` hidden, and the primitive fallback deleted (a missing profile logs an error and draws nothing). **#10 Lane labels:** `CreateLaneLabel`, `CreateLaneOwnershipBadge` and the never-called `CreateLaneEndpointLabel` deleted with the badge rail — the 2-px smudge above every board was "YOUR LINE - DEFEND" in a legacy `TextMesh`. **#9 Floating text:** typed `BoardLabelKind`s, per-cell stacking at 0.35/label, a 0.2 s merge window (ten leaking Swarm now read "−7 LIVES" over one "+11" instead of a pile), one TMP font and outline material resolved from `RuntimeUiChrome`. **#6 Spawn stacking:** presentation-only per-creep lateral offset (±0.30, scaled by footprint, 0 for Colossus, half for hovers) in the creep's own facing frame plus a −0.35-unit longitudinal stagger decaying over 3 s, from the existing digit hash through a murmur finaliser (ids under ~120 otherwise shared one slot). |
| 49 | (this commit) | Icon made with the tool the item said did not exist: `tools/art_pipeline/render_send_icon.py` (Blender, the same script that rendered the ten Foundry/Grove icons in `b46a1ec`) run on the Twin Crescent kitbash FBX. 128×128, alpha bbox 32% fill, inside the range of its siblings. Resolved by name through `TowerCatalog.IconResource`; no code change. |
| Render review Wave 2 | (this commit) | Five items from the same audit, worked in parallel and verified the same way, plus a shader tuning pass and a scale correction caught by the re-capture rather than assumed correct. **#4 Board material:** `LTWBoardVertexColor.shader` gained world-space fractal value noise (albedo break-up, fake normal perturbation, tile variants keyed off the mesh's own 1-unit cell grid), edge darkening, and a path inlay keyed off the existing bright/blue route-guide vertex colours — no UVs, no texture assets (none available in this environment). **#5 Grounding:** creeps drop their contact-shadow decal and owner glow, keep the real cast shadow; hovering creeps get a tight ground-anchor decal instead of an elongated cast shadow; mechanic decals (bramble, grovebond) moved to a dedicated sharper-edged pool at ≥0.5 alpha — Spore Cloud's fog stayed soft on purpose, since its full-radius gradient IS the range indicator by design. Selection-gating was deliberately not built (no hover state in a touch game) — flagged as a decision, not silently dropped. **#11 Tower shadows:** root cause is `Tower3DImportPipeline.NormalizeRendererPolicy` turning shadows off for every renderer under an imported tower, `Body` included, not just accessories — fixed at runtime (`ApplyTowerBodyShadowPolicy`) since regenerating 15 prefabs needs an interactive Editor session; the import-pipeline fix is still owed, see the new item below. Gate sprites get an AO disc reusing the existing `FoundationShadow` recipe; Elder Canopy/Bloomheart get a small per-role Y sink. **#12 Tier silhouettes:** a generic `ApplyTierSilhouette` scales `OwnerTrim`/`RoleMarker`/`RangeHalo` by name at tier 2/3, verified against five shipped prefabs' actual YAML (not just Gatling) to confirm the names are universal. **#13 Backdrop:** a new procedural `LTW/Backdrop` shader (stone plate, vignette, circuit-inlay lines, two lane-accent tones) plus a sparse ambient `ParticleSystem`, sized to the board's real extents and edge-matched to the camera's clear colour so no seam is ever visible. Its first pass shipped a grid bright and dense enough to read as a debug overlay at the shipped camera; retuned directly after the re-capture caught it (glow 1.1→0.32, cell size 4→9, vignette outer radius 0.85→0.32). **#7 Scale:** towers/creeps renormalized from real measured data where it existed (`Tower3DProofSetGenerator`'s footprint solve, `GD_TUNING_LOG.md`'s creep silhouette pass) rather than remapped blindly; one real overcorrection — Barricade 0.71→0.4, clearly undersized next to its neighbours in the re-capture — corrected to 0.55. Builder moved off the lane's unmazed centre column to its edge column, and a genuine "builder not visible anywhere" scare during verification turned out to be a gap in the capture harness itself (`Deselect()` cleared tower selection by reflection without the `UpdateBuilderAvatar()` call the real deselect flow makes), not a product bug — fixed in `GraphicsAuditCaptureRunner.cs`, with a permanent recheck step added so it can't recur silently. |
| Render review Wave 5 | (this commit) | The VFX rebuild, tier silhouettes and the two proofs Wave 4 could not give. Three agents on disjoint files, two new capture steps, three capture passes, compile clean (one fix: the crown cache keyed on `GetInstanceID()`, obsolete-as-error in 6000.5, re-keyed on the instance), 336/336, device export fresh. **#8 / R4 combat VFX:** shots are pooled tapered darts with a fading trail (new `LTW/Combat VFX` additive wedge shader + `CombatVfxResources`), hits and kills a small ring plus 5–7 sparks, the green bracket removed, all four death styles bursts — no crossed beam remains in combat. Tesla's arc, the vine lashes and the muzzle bursts kept. Death hold 0.4 s: scale to 0.15 on an eased curve, 0.3-unit sink, body darkened — proven at `38-kill-seq-09..11` (full, ~40% and sinking with a burst, gone). **#12 tier silhouettes:** a procedural stud crown under the tower root — 4 studs at tier 2, 8 at tier 3, world-sized 0.10/0.12 after a first pass at 0.14/0.16 competed with the selection ring — plus a +25%/+50% body emission lift; tier-3 vs tier-1 Arrows now distinct at the shipped framing. **R5 proof:** `37-leak-row-labels-seq` shows "−N LIVES" drawn over the creep on the gate. **Selection ring:** the opaque disc is an owner-accent annulus at α 0.5 with a slow pulse. Residuals moved to item 53. |
...
| 52 | (this commit) | `NormalizeRendererPolicy`/`ApplyRuntimeMaterial` both flipped from Off/false to On/true — tracing the one call site each has confirmed every renderer either ever touches is Body's own imported visual, generated before any accessory exists, so "exclude Body" (the item's own original guess) was moot; there was nothing else in scope to exclude it from. `ValidateRendererPolicy` updated to match, and its Off/false expectation for the three flat accessories (`RangeHalo`/`RoleMarker`/`OwnerTrim`) left untouched. Applying this to the 16 already-shipped prefabs (not 15 — the item undercounted) used a new `RepairShippedBodyShadowPolicy` menu command rather than "Regenerate ALL" (that item's own menu label says "(discards LODs)"): `LoadPrefabContents`/`SaveAsPrefabAsset` edits the existing prefab in place, so it can only ever flip the two shadow fields on a renderer that already exists, never touch the hand-bound LODGroup or its LOD1/LOD2 renderers structurally. Confirmed those LOD1/LOD2 renderers were already shipped correctly On/true (a hierarchy dump of `Tower_Arrow_3D` before touching anything found them as root-level siblings of Body, never covered by the buggy generator in the first place) — the repair only needed to walk Body's own descendants, and doing so brought `ValidateProofWrappers` from 48 issues to 0, with a git diff on all 16 prefabs of exactly 48 lines (two fields × 24 renderers), nothing structural touched. The runtime override (`ApplyTowerBodyShadowPolicy`) is deleted along with its call site now that the prefabs carry the fix natively; a stale cross-reference to it in `ApplyPulseRangeRingFix`'s own remarks (finding #15) updated rather than left dangling. Unity batchmode compile clean (0 `warning CS`, 0 `error CS`), a `GraphicsAuditCaptureRunner` pass confirmed nothing broke visually. |
| 53 | (this commit) | Audited all four residuals fresh before touching anything — all still reproduced, and all four are now closed (three fixed, one determined to be working as intended). **Label stacking:** `FloatingTextStackStep` raised 0.35→0.55 (`UnityVerticalSliceRenderer.Cues.cs`); a fresh `37-leak-row-labels-seq` capture shows "+25"/"−5 LIVES" as two clearly separated lines instead of the near-total overlap the audit found (worse than the item's own "slightly"). **Hit flash timing:** the fixed-colour flash `RenderEvents` raised unconditionally for every `CreepDamagedEvent` is now read (not cleared — `SpawnCreepHitCue` already consumes the same flag, so `RenderEvents` has to read it first) and only fires immediately when no dart was launched; when one was, `UpdateProjectiles` raises the identical flash at the dart's actual landing instead, since every projectile this system carries traces back to that same event (`SpawnProjectile`/`LaunchProjectile` have no other caller). **Elimination/victory cues:** `SpawnLaneShutdownCue`/`SpawnVictoryLaneCue` replaced their crossed-`SpawnBeam` pairs (an X and a +) with a lane-scaled `SpawnExpandingRingCore` pulse, the same primitive `SpawnCellFrameCue` already uses at cell scale — sized off `LaneWidth` since the camera frames one lane at a time roughly as wide as tall. Not independently screenshotted (a live match's own ~0.6s transient, no capture step currently targets that exact moment), but exercised error-free during a full batch playtest's 7 real eliminations and 1 real victory. **Control ward base dish: not a bug, closed no-op.** A live-scene dump (a temporary diagnostic added to `GraphicsAuditCaptureRunner`, reverted after use) traced the actual tier-2 Control tower's renderers directly rather than trusting the prefab alone: `RangeHalo` is confirmed disabled (matching an earlier session's own finding), `OwnerTrim` is confirmed correctly blue (`ArcaneBlue`, alpha 0.34, correctly tier-scaled) — neither is the violet seen on screen. The violet is `mat_tower_control_3d_body_runtime_v01`'s own `_EmissionColor` (HDR, 1.22/0.85/2.0) and `_RimColor` (0.59/0.52/1.0) blooming under URP Bloom — the tower's own authored material, matching its catalog icon. Git history on that exact material file traces to a commit titled "Split the stylized defaults by role, after watching them in the game," whose own message explains towers "had lost the violet that is the arrow line's identity" and restores it — the Arcane line's (Arrow/Control/Relay/Pulse/Prism/TwinCrescent) violet glow is deliberate brand identity, confirmed unchanged by that commit. Dimming it, as the item requested, would undo that intentional work; left as shipped. Unity batchmode compile clean (0 `warning CS`, 0 `error CS`), batch playtest passed with a clean reset. |
...
**Re-confirmed 2026-09-02** while working render review Wave 3's finding #20, which had
claimed (incorrectly) that Meshy exported normal maps for most of the roster into
`AIStaging`. A full search of all 143 PNGs under `Assets/Art/AIStaging` for that wave's 12
highest-scale candidate units (Tesla, Repair Drone, Spore Cloud, Foundry, Prism and Arrow
towers; Colossus, Obsidianbrute, Warden, Burrower, Siege and Turretwalker creeps) turned up
zero normal-map textures — consistent with this item's 2026-07-31 conclusion, not a
contradiction of it. #20 is not actionable without one of the two routes above; no code
changed.
