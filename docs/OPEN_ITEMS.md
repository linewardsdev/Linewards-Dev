...
Opened 2026-09-02 while fixing render review finding #11. `NormalizeRendererPolicy` sets
`shadowCastingMode = Off` and `receiveShadows = false` on every `MeshRenderer` under an imported
tower's root — correct for thin accessory pieces (`RangeHalo`, `OwnerTrim`, `RoleMarker`, the
per-role rings/dishes), which read badly with a shadow, but it also silently caught `Body`, the
one renderer that should cast one. Fixed for the shipped build at runtime instead
(`UnityVerticalSliceRenderer.TowerPresentation.cs`'s `ApplyTowerBodyShadowPolicy`, called from
`SetTowerTransform`), because the correct fix belongs in the generator and regenerating all 15
tower prefabs from it needs an interactive Unity Editor session, not the batchmode this work was
done under.

**What it needs:** change `NormalizeRendererPolicy` to exclude `Body` (by name-matching against
the same renderer the runtime fix already targets), regenerate all 15 tower prefabs, and delete
the now-redundant runtime override once the prefabs carry the fix natively. Also update
`ValidateRendererPolicy` in the same file, which currently *enforces* Off/false on Body-class
renderers and would flag a regenerated prefab as wrong.
