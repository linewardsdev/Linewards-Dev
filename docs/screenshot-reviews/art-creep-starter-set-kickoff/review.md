...
| Resolved | Build/send menu text crowding | `captures/02-build-menu-open.png` and `03-send-menu-open.png` now place expanded 5-option menus above the bottom dock using a two-line 3+2 card hierarchy. | Keep expanded menus above the launcher row; use iconography later if more options are added. |
...
- `captures/02-build-menu-open.png`: Build palette opens above the launcher row; all five tower labels/costs are readable at the captured mobile-safe size.
- `captures/03-send-menu-open.png`: Send dock opens above the launcher row, stays on screen, and all five send cards are readable.
